...
//       return array('slider'=>$sliderData,'serviceArr'=>$serviceArr,'category'=>$categoryArr,'product'=>$productArr,'cart'=>$cartData);
    ...
    function addToFavourite($data){
        $favData=$this->getSingleDataRow('wishlist','user_id="'.$data['user_id'].'" and product_id="'.$data['product_id'].'"');
        if($favData){
            $delete=$this->db->where("user_id", $data['user_id'])
                   ->where("product_id", $data['product_id'])
                   ->delete("wishlist");
            if($delete){
                return array('status'=>true,'code'=>200,'message'=>'Remove from favourite.','data'=>array());
            }else{
            ...
            if($insetFav){
                return array('status'=>true,'code'=>200,'message'=>'Add to favourite.','data'=>array());
            }else{
                return array('status'=>false,'code'=>201,'message'=>'Some error found.','data'=>array());
            }
        }
...
//            print_r($products);exit;
        ...
        function getAds($data){
            $slider = $this->db->select("slider.*,u.name as vendor_name")
