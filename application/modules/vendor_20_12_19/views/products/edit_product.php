        ...
        <form id="Franchisee" method="post" class="right-text-label-form" enctype="multipart/form-data">
            <div class="container-fluid">
                ...
                <div class="row">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control validate" id="sub_category" name="sub_categorys" onchange="subCatTitle(this);">
                                                ...
                                                foreach ($category as $rows) {
                                                    if ($product['category_id'] == $rows['id']) {
                                                        foreach ($rows['subCategory'] as $sub) {
                    ...
                    <div class=" col-md-12" id="productAttributesDiv" style="display: <?= $attributes ? 'block' : 'none'; ?>;">
                        ...
                        <div class="card card-shadow mb-4">
                            ...
                            <div class="card-body">
                                <div class="row" id="attributeData">
                                    <?php foreach ($attributes as $attr): ?>
                                        <div class="col-sm-4">
                                            <div class="form-group">
                                                <label for="name"><?= $attr['title'] ?></label>
                                                <span class="error text-danger" id="attribute_value_<?= $attr['id'] ?>"></span>
                                                <input type="hidden" value="<?= $attr['id'] ?>" name="attribute_id[]"/>
                                                <select class="form-control <?= $attr['is_required'] ? 'validate' : '' ?>" name="attribute_value_<?= $attr['id'] ?>">
                                                    <option selected value="">Select <?= $attr['title'] ?></option>
                                                    <?php
                                                    foreach ($attr['values'] as $value):
                                                        ...
                                                        <?php
                                                        if ($product['feature']):
                                                            foreach ($product['feature'] as $feature):if ($feature['attribute_id'] == $attr['id']):
                                                                    if ($feature['attribute_value_id'] == $value['id']): echo 'selected';
                                                                    else: echo 'disabled';
                                                                    endif;
                                                                endif;
                                                            endforeach;
                    ...
                    <div class=" col-md-12" id="productSpecificationDiv" style="display: <?= $specification ? 'block' : 'none'; ?>">
                        ...
                        <div class="card card-shadow mb-4">
                            ...
                            <div class="card-body">
                                <div class="row" id="attributeSpecData">
                                    <?php foreach ($specification as $spc): ?>
                                        <div class="col-sm-4">
                                            <div class="form-group">
                                                <label for="name"><?= $spc['title'] ?></label>
                                                <span class="error text-danger" id="specification_value_<?= $spc['id'] ?>"></span>
                                                <input type="hidden" value="<?= $spc['id'] ?>" name="specifications[]"/>
                                                <input type="text"
                                                       value="<?php
                                                       if ($product['specification']):
                                                           foreach ($product['specification'] as $specifications):if ($specifications['attribute_id'] == $spc['id']):
                                                                   echo $specifications['value'];
                                                               endif;
                                                           endforeach;
                                                       endif;
                                                       ?>"
                                                       name="specification_value_<?= $spc['id'] ?>" placeholder="Enter <?= $spc['title'] ?>" class="form-control  <?= $spc['is_required'] ? 'validate' : '' ?>" id="attributeSpec_name">
                                            </div>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
                                    ...
                                    <div class="form-group row titleeventimage">
                                        ...
                                        for ($i = 0; $i < 4; $i++):
                                            ...
                                            <?php else: ?>
                                                ...
                                                <div class="col-sm-3 file-upload">
                                                    <img id="blah<?= $i + 1 ?>" src="<?php echo base_url(); ?>assets/vendor/common/images/logo/dummy.jpg" alt="your image" />
                                                    <label for="upload<?= $i + 1 ?>" class="file-upload__label">Upload Image <?= $i + 1 ?></label>
                                                    <input id="upload<?= $i + 1 ?>" class="file-upload__input" type="file" name="file_upload[]" accept=".jpg,.png,.jpeg" onchange="readURL(this, <?= $i + 1 ?>);">
                                                </div>
                                            <?php endif;endfor; ?>
...
//                                            var formData = $("#Franchisee").find('select');
