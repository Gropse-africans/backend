...
class Admin extends CI_Controller {
    ...
    public function user_detail() {
        ...
        if ($user) {
            $order = $this->Admin_model->getTableDataArray('orders', 'status!=0 and user_id='.$id, 'order_id', '');
            $data['order'] = $order;
            $booking = $this->Admin_model->getBooking(['b.status!=' => 0,'b.user_id'=>$id])->result_array();
...
//            echo '<pre/>';print_r($data);exit;
    ...
    public function advertisment() {
        $sliderArr = array();
        $slider = $this->Admin_model->getTableDataArray('slider', ['status!='=>99], 'id', '');
        if ($slider) {
...
//            print_r($insertArr);exit;
    ...
    {
        $support = $this->Admin_model->help_support();
        // echo "<pre>";print_r($support);exit;
        $data['support'] = $support;
        $data['view_link'] = 'help_Support';
        $this->load->view('layout/template', $data);
    }
